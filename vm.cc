    ...
    if (sregs.cs.base == 0xf0000) {
        ...
        switch (intr_nr) {
            ...
            case 0x16:
                ...
                if (*(uint16_t *)&full_mem[0x21 * 4 + 2] != 0xf000) {
                    dos21_offset = *(uint16_t *)&full_mem[0x21 * 4 + 0];
                    dos21_seg = *(uint16_t *)&full_mem[0x21 * 4 + 2];
                    *(uint16_t *)&full_mem[0x21 * 4 + 0] = 0x21;
                    *(uint16_t *)&full_mem[0x21 * 4 + 2] = 0xf000;
                    puts("replace dos21");
                }
...
static void disasm(int fd, unsigned char *full_mem) {
    ...
    printf("rip=%x, cs=%x, ds=%x flags=%08x\n", regs.rip, sregs.cs.base,
           sregs.ds.base, regs.rflags);
...
int main(int argc, char **argv) {
    ...
    // }
    memset(full_mem, 0x00, 1024 * 1024);
    ...
    int image_fd = open("floppy", O_RDONLY);
    if (image_fd < 0) {
